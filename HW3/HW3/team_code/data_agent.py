...
class DataAgent(AutoPilot):
	...
	def setup(self, path_to_conf_file, route_index=None):
		...
		if self.save_path is not None and self.datagen:
			...
			# (self.save_path / 'semantics_rear').mkdir()
			(self.save_path / 'semantics_augmented').mkdir()
			(self.save_path / 'depth').mkdir()
			# (self.save_path / 'depth_rear').mkdir()
			...
			(self.save_path / 'birdview').mkdir()
			(self.save_path / 'birdview_npy').mkdir()
	...
	def run_step(self, input_data, timestamp, sensors=None, plant=False):
		...
		# interfuser BEV
		self.birdview_interfuser_np = self.birdview_producer.produce(agent_vehicle=self._vehicle)
		self.birdview_interfuser = BirdViewProducer.as_rgb(self.birdview_interfuser_np)
	...
	def save_sensors(self, tick_data):
		...
		# HD map from transfuser
		img = encode_npy_to_pil(np.asarray(tick_data['topdown'].squeeze().cpu()))
		img_save=np.moveaxis(img,0,2)
		cv2.imwrite(str(self.save_path / 'hdmap' / ('encoded_%04d.png' % frame)), img_save)


		### save interfuser bird eye view 
		Image.fromarray(self.birdview_interfuser).save(self.save_path / 'birdview' / ('%04d.png' % frame))
		# np.save( self.save_path / 'birdview_npy' / ("%04d.npy" % frame), self.birdview_interfuser_np, allow_pickle=True)
		np.savez_compressed(self.save_path / 'birdview_npy' / ("%04d" % frame), self.birdview_interfuser_np, allow_pickle=True) # store with npz in compressed format will save much of space on disk (1.4MB/frame->3.8KB/frame)
		...
		# release cuda memory occupied by HD map renderer
