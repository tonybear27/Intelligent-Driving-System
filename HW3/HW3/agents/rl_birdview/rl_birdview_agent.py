...
class RlBirdviewAgent():
    ...
    def setup(self, path_to_conf_file):
        ...
        if cfg['wb_run_path'][-1] == '\n':
            print(cfg['wb_run_path'][:-1])
            cfg['wb_run_path'] = cfg['wb_run_path'][:-1]
    ...
    def learn(self, env, total_timesteps, callback, seed, ckpt_path_and_run_path):
        if self._policy is None:
            self._policy = self._policy_class(env.observation_space, env.action_space, **self._policy_kwargs)

        # init ppo model
        # print(self._train_cfg['entry_point']) agents.rl_birdview.models.ppo:PPO
        model_class = load_entry_point(self._train_cfg['entry_point'])
        model = model_class(self._policy, env, **self._train_cfg['kwargs'])
        model.learn(total_timesteps, callback=callback, seed=seed, ckpt_path_and_run_path=ckpt_path_and_run_path)
