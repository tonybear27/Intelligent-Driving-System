...
class BevEncoder(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.perspective_upsample_factor = self.image_encoder.feature_info.info[
        self.img_start_index + 2]['reduction'] // self.config.perspective_downsample_factor

    # Delete unused layer, so we don't have to search for unused parameters.
    name = self.image_encoder.feature_info.info[self.img_start_index + 3]['module']
    delattr(self.image_encoder, name)
    
    num_img_features_1 = self.image_encoder.feature_info.info[self.img_start_index + 1]['num_chs']
    num_img_features_2 = self.image_encoder.feature_info.info[self.img_start_index + 2]['num_chs']
    ...
    if self.use_lidar_branch:
      ...
      # Delete unused layer, so we don't have to search for unused parameters.
      name = self.bev_encoder.feature_info.info[self.img_start_index + 3]['module']
      ...
      if self.config.use_bev_semantic or self.config.use_bev_topo or self.config.detect_boxes:
  ...
  def forward(self, image, lidar):
    ...
    # if we only predict representation from image branch at 1st stage training, we don't need to project image features to BEV space and fused with lidar feature
    if self.use_lidar_branch:
