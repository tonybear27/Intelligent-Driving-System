...
class CARLA_Data(Dataset):  # pylint: disable=locally-disabled, invalid-name
				...
				for seq in range(config.skip_first, num_seq - self.config.pred_len - self.config.seq_len):
					...
					for idx in range(self.config.seq_len):
						...
						if not self.config.use_plant:
							...
							topdown.append(route_dir + '/birdview_npy' + (f'/{(seq + idx):04}.npz'))
	...
	def __getitem__(self, index):
		...
		for i in range(self.config.seq_len):
			...
			if not self.data_cache is None and cache_key in self.data_cache:
				...
				if not self.config.use_plant:
					...
					if self.config.use_hdmap:
						hdmaps_i.seek(0)###tf
						hdmaps_i = np.load(hdmaps_i)['arr_0']###tf
			...
			# Load data from the disc
				...
				if not self.config.use_plant:
					...
					if self.config.use_hdmap:
						hdmap_array = cv2.imread(str(hdmaps[i], encoding='utf-8'), cv2.IMREAD_UNCHANGED)
						if (hdmap_array is None):
							print("Error loading file: ", str(hdmaps[i], encoding='utf-8'))
						hdmap_array = np.moveaxis(hdmap_array, -1, 0)
						hdmaps_i = decode_pil_to_npy(hdmap_array).astype(np.uint8)
					if self.config.augment:
						...
						if self.config.use_vehicle_occupancy:
							vehicles_occupancy_i, _ = get_labels(topdowns[i])
						if self.config.use_pedestrian_occupancy:
							_, pedestrians_occupancy_i = get_labels(topdowns[i])
				...
				if not self.data_cache is None:
					...
					if not self.config.use_plant:
						...
						if self.config.use_hdmap:
							compressed_hdmaps = io.BytesIO()  # bev has 2 channels which does not work with png compression so we use generic numpy in memory compression
							np.savez_compressed(compressed_hdmaps, hdmaps_i)
						if self.config.augment:
			...
			if self.config.use_hdmap:
				loaded_hdmaps.append(hdmaps_i)
			if self.config.use_vehicle_occupancy:
				loaded_vehicles_occupancy.append(vehicles_occupancy_i)
			if self.config.use_pedestrian_occupancy:
				loaded_pedestrians_occupancy.append(pedestrians_occupancy_i)
		...
		if not self.config.use_plant:
			...
			if self.config.use_hdmap:
				hdmaps_i = load_crop_hdmap_npy(loaded_hdmaps[self.config.seq_len-1],0) #tf+
				data['hdmap'] = hdmaps_i #tf+
			# The transpose change the image into pytorch (C,H,W) format
			data['rgb'] = np.transpose(processed_image, (2, 0, 1))
			if self.config.use_vehicle_occupancy:
				vehicles_occupancy_i = loaded_vehicles_occupancy[self.config.seq_len-1]
				data['vehicle_occupancy'] = (torch.from_numpy(vehicles_occupancy_i).unsqueeze(0).unsqueeze(0))
			if self.config.use_pedestrian_occupancy:
				pedestrians_occupancy_i = loaded_pedestrians_occupancy[self.config.seq_len-1]
				data['pedestrian_occupancy'] = (torch.from_numpy(pedestrians_occupancy_i).unsqueeze(0).unsqueeze(0))
...
def decode_pil_to_npy(img):
	...
	hdmap_array = np.zeros([channels, width, height])

	for ix in range(5):
			bit_pos = 8-ix-1
			hdmap_array[[ix, ix+5, ix+5+5]] = (img & (1<<bit_pos)) >> bit_pos

	# hard coded to select
	return hdmap_array[10:12]  


def load_crop_hdmap_npy(bev_array, degree):
	"""
